...
public class ProductController {
    ...
    public void addToShoppingCart(ArrayList<Product> shoppingCart, Product product, int numberOfProduct) throws NoEnoughSupplyException {
        if (numberOfProduct <= product.getNumberOfProduct()) {
            for (int i = 0; i < numberOfProduct; ++i)
                shoppingCart.add(product);
        } else throw new NoEnoughSupplyException();
    }
